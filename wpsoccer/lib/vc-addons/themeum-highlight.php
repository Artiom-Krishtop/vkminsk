	...
	if ($style == 'style1') {
	    ...
	    if(count($posts)>0){
		    ...
		    if($i==1){
			    ...
			    $output .= '<a href="'.get_permalink().'">'.get_the_post_thumbnail($post->ID, array(694, 456), array('class' => 'img-responsive')).'</a>';
				...
				$output .= '<h3 class="entry-title"><a href="'.get_permalink().'">'. get_the_title() .'</a></h3>';
				//$output .= '<span class="author"><i class="fa fa-edit"></i>'.__('By ', 'themeum').''.get_the_author_link().'</span>'; // ITGarant edit
				if ( $show_date == 'yes'){
				...
				$output .= '<a href="'.get_permalink().'">'.get_the_post_thumbnail($post->ID, array(109, 95), array('class' => 'img-responsive')).'</a>';
				if ( $show_category == 'yes'){
					$output .= '<span class="entry-category">';
					$output .= get_the_category_list(', ');
					$output .= '</span>';
